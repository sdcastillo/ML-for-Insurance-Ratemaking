---
title: "Comparing Machine Learning Models for Insurance Ratemaking"
author: "Sam Castillo"
...
p2 <- car %>% 
  ggplot(aes(exposure)) + 
  geom_histogram() + 
  ggtitle("Exposure: Length of Coverage Term") +
  theme_light()
...
We see that the most severe accidents are for family vehicles and daily drivers, such as trucks, sedans, and hatchbacks.  The laest severe claims are for weekend cars such as sports cars, hard tops, and busses.  These are vehicles that do not get driven as often.
...
car %>% 
  ...
  coord_flip() + 
  ggtitle("Average Severity for Non-Zero Claims by Vehicle Body Type") + 
...
car %>% 
  ...
  summarise(mean_frequency = sum(numclaims/exposure, na.rm = T)/n(),
            veh_value = mean(veh_value, na.rm = T)) %>% 
  ggplot(aes(y = mean_frequency, x = veh_body, fill = veh_value)) + 
  geom_bar(stat = "identity") + 
  coord_flip() + 
  ggtitle("Average Frequency for Non-Zero Claims by Vehicle Body Type")+
  scale_x_discrete(limits = severity_order) +
  theme_light()
...
The most severe accidents total the car because there are no subsequent claims.  What happens when the claim cost is greater than the vehicle's value?  When a vehicle is totaled, the policy can have no additional claims.  This means that the majority of the large losses will occur with only one claim on the policy. 

```{r}
car %>% 
  filter(claimcst0 > 0, veh_value > 0) %>% 
  mutate(totaled = ifelse(claimcst0 > veh_value*10000, "totaled", "not_totaled")) %>% 
  ggplot(aes(veh_value, claimcst0, color = totaled)) + 
  geom_point() + 
  xlim(0, 5) + 
...
car %>% 
  ...
  corrplot(
    ...
    sig.level = 0.01, 
    insig = "blank", 
    title = "Correlations in Claim Severity",
...
* Took a 50% sample of the data in order to speed up the modeling process. This is only because my laptop is weak.
...
```{r}
set.seed(1)
severity_train_index <- createDataPartition(severity$avg_capped_losss, times = 1, p = 0.7, list = FALSE) %>% as.vector()
...
frequency_train_index <- createDataPartition(car$numclaims, times = 1, p = 0.7, list = FALSE) %>% as.vector()
...
CAS GLM Paper: https://www.casact.org/pubs/monographs/papers/05-Goldburd-Khare-Tevet.pdf

CAS Presentation: https://www.casact.org/education/rpm/2012/handouts/Session_4738_presentation_1068_0.pdf
